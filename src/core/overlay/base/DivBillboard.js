...
class DivBillboard extends Overlay {
  ...
  _updateStyle(style, distance,resolution) {
    let translate3d = 'translate3d(0,0,0)'
    // let width=style.width?style.width:'200'
    if (style.transform) {
    ...
    this._delegate.style.margin="10px"
  }
  ...
  _addHandler(layer) {
    ...
    this._delegate.addEventListener('click', e => {
      that._overlayEvent.fire(DC.MouseEventType.CLICK, {
        layer: layer,
        overlay: that,
        position: Transform.transformWGS84ToCartesian(that._position)
      })
      that._overlayEvent.fire(DC.MouseEventType.DRAG_START, {
        layer: layer,
        overlay: that,
        position: Transform.transformWGS84ToCartesian(that._position)
      })
      that._overlayEvent.fire(DC.MouseEventType.DRAG_END, {
        layer: layer,
        overlay: that,
        position: Transform.transformWGS84ToCartesian(that._position)
      })
